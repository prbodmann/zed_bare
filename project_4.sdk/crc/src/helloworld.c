...
#define FILE_SIZE 26611200
unsigned char fbuffer[FILE_SIZE];
int crc32file(unsigned int *crc)
{
      register unsigned int oldcrc32,i;


      oldcrc32 = 0xFFFFFFFF;

      for(i=0;i<FILE_SIZE;i++){
               oldcrc32 = UPDC32((unsigned int)fbuffer[i], oldcrc32);
           }
...
int main()
{
    ...
    //--------------------------------------------
    //Disable PCAP
    static XDcfg g_DcfgInstance;
    XDcfg *DcfgInstPtr;
    XDcfg_Config *ConfigPtr;
    DcfgInstPtr = &g_DcfgInstance;
    ...
    if (Status != XST_SUCCESS) {
        //xil_printf("XDcfg_CfgInitialize failed \n\r");
        return -1;
    }
    XDcfg_SelectIcapInterface(&g_DcfgInstance);

    //print("XDcfg_SelectIcapInterface() done.\n\r");
    //-----------------------------


    if ((rc = f_mount(&fatfs,"0:/",0)) != FR_OK) {
	   xil_printf("main(): f_mount(0:) failed with return code %d!\r\n",rc);
	   return(-1);
    }
    if ((rc = f_open(&fin,"0:/large.pcm",FA_OPEN_EXISTING | FA_READ)) != FR_OK) {
       xil_printf("main(): f_open() failed with return code %d!\r\n",rc);
       return 0;
	 }
	 f_read(&fin,fbuffer,FILE_SIZE,&bytes_read);
     f_close(&fin);
   //int count;
